...
public sealed class GoogleAnalyticsHelper : MonoBehaviour
{
    ...
    public void Awake()
    {
        if (instance == null && instance != this)
        {
            ...
            GoogleAnalyticsHelper.Settings("UA-37421441-4", "http://loveland.za.org");
            logEvent("game", "start", 0);
            logEvent("platform", SystemInfo.deviceModel.ToString() + "+" + SystemInfo.operatingSystem.ToString(), 0);
        }
    ...
    public static void logEvent(string category, string action, int value)
    {
        ...
        {
            GoogleAnalyticsHelper.LogEvent("", "SokoBomber", category, action, value);
        }
    }
    ...
    public static void LogEvent(string page, string category, string action, string opt_label, int opt_value)
    {
        ...
        /// Execute query:
//#if UNITY_WEBPLAYER
//        Application.ExternalCall("gaLogEvent", "SokoBomberWebPlayer", "" + action, "" + opt_label, opt_value, true);
//#else
        new WWW(url);
//#endif
    }
